...
router.post(
  ...
  error_catcher(async (req, res) => {
    ...
    const receiveShareTriggers = Trigger.find({ when_trigger: "ReceiveMobileShareData" });
    let allowShareTo = receiveShareTriggers.length > 0;
    if (allowShareTo && !shareProvisioningProfile) {
      allowShareTo = false;
      msgs.push({
        type: "warning",
        text: req.__("A ReceiveMobileShareData trigger exists, but no Share Extension Provisioning Profile is provided."),
      });
    }
